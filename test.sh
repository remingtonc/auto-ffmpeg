...
for VIDEO_FILE_PATH in $SOURCE_PATH/*.mkv; do
	...
	docker run --rm -v $SOURCE_PATH:/data/source -v $TARGET_PATH:/data/target ffmpeg -i "/data/source/${VIDEO_FILENAME}" -c:v libx265 -crf 20 -preset slow -c:a libfdk_aac -vbr 5 "/data/target/${VIDEO_FILENAME}"
